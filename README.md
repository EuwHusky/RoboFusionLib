...
RM常用功能模块，比如：底盘（行为控制，功率控制）、云台（行为控制，发射控制）等，内置外设控制功能。

### 算法层`algorithm`（开发中）

常用数据处理、数据结构、控制器、机器人运动学等算法（纯软件、不与硬件相关），移植不会报错，不内置外设控制功能。
...
2. 结构体&枚举数据类型命名，使用小写下划线法，结构体要加后缀`_s`，枚举要加后缀`_e`，例如`rfl_example_struct_s`、`rfl_example_enum_e`；
