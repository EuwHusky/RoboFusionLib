...
void rfl_motor_pid_no_force_control(rfl_motor_s *motor)
{
...
void rfl_motor_pid_speed_control(rfl_motor_s *motor)
{
...
void rfl_motor_pid_angle_control(rfl_motor_s *motor)
{
    motor->max_speed_ = 0.0f;
    rflAngleUpdate(&motor->track_angle, RFL_ANGLE_FORMAT_DEGREE, 0.0f);
...
void rfl_motor_pid_direction_control(rfl_motor_s *motor)
{
    if (motor->controller_type == RFL_MOTOR_CONTROLLER_PID)
    {
        motor->max_speed_ = 0.0f;
...
void rfl_motor_pid_speed_angle_control(rfl_motor_s *motor)
{
    /* 角度限幅 */
    if (motor->set_angle_.deg > motor->max_angle_.deg)
        rflAngleUpdate(&motor->set_angle_, RFL_ANGLE_FORMAT_DEGREE, motor->max_angle_.deg);
...
void rfl_motor_unitree_angle_control(rfl_motor_s *motor)
{
    motor->max_speed_ = 0.0f;
...
void rfl_motor_unitree_speed_angle_control(rfl_motor_s *motor)
{
    /* 角度限幅 */
    if (motor->set_angle_.deg > motor->max_angle_.deg)
        rflAngleUpdate(&motor->set_angle_, RFL_ANGLE_FORMAT_DEGREE, motor->max_angle_.deg);
